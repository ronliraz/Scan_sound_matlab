...
for f = 1 : length(folder_names)
    ...
    all_data=[];all_label=[];
    for f = 1 : length(folder_names)
        load_data_temp = load(folder_names{f});
...
for f = 1 : length(labels_name)
    data_temp = all_data(strcmp(all_label,labels_name{f}));
    for c = 1 : length(data_temp)
        cell_temp = data_temp{c};
        while length(cell_temp) >= min_len
            data_out{ii} = cell_temp(1:min_len);
            label_out{ii} = labels_name{f};
            cell_temp(1:min_len) = [];
            ii = ii+1;
        end
    end
end
%sound(data_out{2},16e3)
figure()
h_new = histogram(categorical(label_out));
%% cutting data and spliting:

min_count = min(h_new.Values);
...
for f = 1 : length(labels_name)
    data_temp = data_out(strcmp(label_out,labels_name{f}));
...
%% array to table:
XTrain = cell2mat(X.train).';
YTrain = categorical(Y.train).';

NumTrees = 20;
B = TreeBagger(NumTrees,XTrain,YTrain,'OOBPred','On','Method','classification');
%%
...
Yfit=oobPredict(B);
yfit_test = predict(B,cell2mat(X.test).');
abs(Y.test-cell2mat(yfit_test).')/numel(yfit_test);
% view trees
